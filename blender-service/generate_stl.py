...
def _split_segments(points):
    ...
    segs = [s for s in _split_by_discontinuity(P, mult=8.0, floor=0.002) if len(s) >= 3]
    return segs
...
def compose_beardline_uniform(segments, sample_count=400, eps=1e-6):
    ...
    out, last_x = [], None
    for x in xs:
        candidates = [p for seg in segments if (p:=_interp_on_segment(seg, x)) is not None]
        if candidates:
            px, py, pz = max(candidates, key=lambda q: q[1])  # highest Y
        else:
...
def _unify_params(params_any):
    ...
    use("lipSegments","lipsegments"); use("arcSteps","arcsteps")
    use("maxLipRadius","maxlipradius"); use("minLipRadius","minlipradius")
    use("taperMult","tapermult"); use("extrusionDepth","extrusiondepth")
    use("weldEps","weldeps"); use("minFeature","minfeature")
...
# ---------------------------
# Front sheet consolidation & extrusion
# ---------------------------
...
def make_mesh_from_tris(tris, name="MoldMesh", weld_eps=WELD_EPS_DEFAULT):
    ...
    for (i, j, k) in faces_idx:
        ...
        uniq.add(fkey); out_faces.append((i, j, k))
...
def _snap_close_endpoints(sorted_pts, tol=1e-4):
    if len(sorted_pts) > 2:
        ...
        if math.dist(a, b) < tol: sorted_pts[-1] = (a[0], a[1], a[2])
    return sorted_pts
...
def main():
    ...
    if neckline_in:
        ...
        neck_segs = _split_segments(neck_pts)
        neck = max(neck_segs, key=len) if neck_segs else neck_pts
