# krezz_mold.py — arc-length sweep (no X-sort), smooth top-edge roll, watertight solid
...
def _resample_by_t(points, ts):
    if not points: return [(0.0,0.0,0.0) for _ in ts]
    P=points[:]; L,total=_cumlen(P)
    if total<=1e-12:
        y=sum(p[1] for p in P)/len(P); z=sum(p[2] for p in P)/len(P)
        return [(p[0],y,z) for p in P]
    out=[]; i=0
    for t in ts:
        s=t*total
        while i<len(L)-2 and L[i+1]<s: i+=1
        a,b=P[i],P[i+1]; s0,s1=L[i],L[i+1]
        u=0.0 if s1==s0 else (s-s0)/(s1-s0)
        out.append((a[0]*(1-u)+b[0]*u, a[1]*(1-u)+b[1]*u, a[2]*(1-u)+b[2]*u))
    return out
...
def densify_by_dx(points,max_dx):
    ...
    for i in range(len(P)-1):
        a,b=P[i],P[i+1]; dx=max(1e-12,b[0]-a[0]); n=max(1,int(math.ceil(dx/max_dx)))
        for k in range(1,n+1):
            t=k/n
            out.append((a[0]+t*dx, a[1]*(1-t)+b[1]*t, a[2]*(1-t)+b[2]*t))
    return out
...
def make_mesh_from_tris(tris,name="BeardMold",weld_eps=WELD_EPS_DEFAULT):
    v2i,verts,faces={},[],[]
    ...
    mesh=bpy.data.meshes.new(name); mesh.from_pydata([Vector(v) for v in verts],[],faces)
...
def create_cylinders_z_aligned(holes,thickness,radius=0.0015875,embed_offset=0.0025):
    cylinders=[]
    for h in holes:
        x,y,z=to_vec3(h); depth=float(thickness); center_z=z-(embed_offset+depth/2.0)
...
def grid_to_tris(grid):
    if not grid: return []
    rows=len(grid); cols=min(len(r) for r in grid); faces=[]
    for r in range(rows-1):
        A=grid[r]; B=grid[r+1]
        for c in range(cols-1):
            a=A[c]; b=A[c+1]; c0=B[c]; d=B[c+1]
            faces.append([a,c0,b]); faces.append([b,c0,d])
    return faces
...
def build_triangles(beardline,neckline,params):
    ...
    profile_bias=float(params.get("profileBias",2.2))
    ...
    edgeR=float(params.get("edgeLipRadius",0.0015))
    edgeAngDeg=float(params.get("edgeLipAngleDeg",35.0))
    edgeSteps=int(params.get("edgeLipSteps",4))

    # >>> FIX: keep original order for arclen; only X-sort in legacy "x" mode
    if sample_mode=="arclen":
        P=[tuple(p) for p in beardline]                # preserve order
        ts=[i/max(1,lip_segments-1) for i in range(lip_segments)]
        base=_resample_by_t(P,ts)
        if base_smooth>0: base=smooth_row_keep_x(base,passes=base_smooth)
        xs=[b[0] for b in base]
    else:
        P=sorted(beardline,key=lambda p:p[0])          # legacy X-sorted
        xs_lin=[P[0][0]+(P[-1][0]-P[0][0])*i/max(1,lip_segments-1) for i in range(lip_segments)]
        base=resample_polyline_by_x(P,xs_lin)
        if (P[-1][0]-P[0][0])/max(1,len(base)-1)>target_dx:
            base=densify_by_dx(base,target_dx)
        if base_smooth>0: base=smooth_row_keep_x(base,passes=base_smooth)
        xs=[b[0] for b in base]

    minX=min(b[0] for b in base); maxX=max(b[0] for b in base); centerX=0.5*(minX+maxX)

    neck_row=[]
    if neckline:
        if sample_mode=="arclen":
            neck_row=_resample_by_t([tuple(p) for p in neckline],[i/max(1,len(base)-1) for i in range(len(base))])  # keep order
            neck_row=smooth_vertices_open(neck_row,passes=3)
        else:
            neck_row=resample_polyline_by_x(neckline,xs); neck_row=smooth_vertices_open(neck_row,passes=3)

    grid=[]
    if edgeR>0.0 and edgeSteps>0 and edgeAngDeg>0.0:
        phi=math.radians(max(0.0,min(85.0,edgeAngDeg)))
        for k in range(edgeSteps,0,-1):
            t=k/float(edgeSteps); ang=phi*t
            row=[(bx, by+edgeR*math.sin(ang), bz+edgeR*(1.0-math.cos(ang))) for (bx,by,bz) in base]
            grid.append(smooth_row_keep_x(row,passes=max(1,lip_smooth_pass//2)))

    grid.append(base)

    for j in range(arc_steps):
        tt=(j+1)/float(arc_steps+1); tt=max(1e-6,min(1.0,tt))**max(0.1,float(profile_bias))
        ang=0.5*math.pi*tt
        row=[]
        for (bx,by,bz) in base:
            dx=abs(bx-centerX); taper=max(0.0,1.0-dx*taper_mult)
            r=min_lip_radius+taper*(max_lip_radius-min_lip_radius)
            y=by - r*math.sin(ang)
            z=bz + prelift*(1.0-math.cos(ang))
            row.append((bx,y,z))
    ...
    if neck_row:
        A=base; B=neck_row; m=min(len(A),len(B))
        for i in range(m-1):
...
def _unify_params(params_any):
    ...
    for cam in ["lipSegments","arcSteps","maxLipRadius","minLipRadius","taperMult",
                ...
                "holeRadius","profileBias","prelift","targetDx","lipSmoothPasses",
                "baseSmoothPasses","sampleMode","edgeLipRadius","edgeLipAngleDeg",
                "edgeLipSteps","autoRemesh"]:
        use(cam,cam.lower())
    return out

def _snap_close_endpoints(pts,tol=1e-4):
    if len(pts)>2:
        a,b=pts[0],pts[-1]
        dx=a[0]-b[0]; dy=a[1]-b[1]; dz=a[2]-b[2]
        if (dx*dx+dy*dy+dz*dz)**0.5<tol: pts[-1]=(a[0],a[1],a[2])
    return pts
...
def main():
    ...
    # >>> keep original order here; no X-sorting
    ...
    if voxel_size>0: clean_mesh(mold_obj,weld_eps,min_feature=params.get("minFeature"),strong=True)
    ...
    if holes_in:
        ...
        apply_boolean_difference(mold_obj,cutters); clean_mesh(mold_obj,weld_eps,min_feature=params.get("minFeature"),strong=True)
