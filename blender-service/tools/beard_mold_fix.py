...
"""
Builds a solid, watertight mold surface from beardline/neckline input.
Key fixes for slicer "gaps":
- Steeper start of the lip (profileBias, prelift) to avoid long shallow overhangs.
- Optional internal anchor ribs (enableAnchorRibs + params) to support top skin.
- Shared-X resampling avoids T-junctions between stitched bands.
Units: meters (0.001 m = 1 mm).
"""
...
# ---------------------------
# Lip generation (with overhang control)
# ---------------------------

def generate_lip_rings(base_points, arc_steps, min_r, max_r, centerX, taper_mult, *, profile_bias=1.35, prelift=0.0):
    # why: profile_bias>1 steepens first segment; prelift creates a short near-vertical start.
...
# ---------------------------
# Robust stitching (shared X samples)
# ---------------------------
...
def extrude_surface_z_solid(tri_faces, depth):
    # why: weld vertices up front to avoid T-junction pinholes.
...
def add_anchor_ribs(mold_obj, xs, params, thickness, lip_band_y):
    ...
    rib_band_y    = float(params.get("ribBandY",     0.004))   # 4 mm
...
def main():
    ...
    # optional: internal ribs before remesh so remesh bakes them in
