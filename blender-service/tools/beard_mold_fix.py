...
# Exports STL in mm by default (global_scale=1000).
...
# ---------------------------
# Lip generation (steeper start + tiny prelift)
# ---------------------------
...
def generate_lip_rings(base_points, arc_steps, min_r, max_r, centerX, taper_mult, *, profile_bias=1.35, prelift=0.0):
    ...
    for (bx, by, bz) in base_points:
        ...
        for j in range(ring_count):
            t = j/float(arc_steps)
            t = pow(t, max(1.0, float(profile_bias)))  # steeper near start
            angle = math.pi * t
...
# ---------------------------
# Robust stitching (shared X samples to avoid T-junctions)
# ---------------------------
...
# ---------------------------
# Solid extrusion & mesh build (weld first)
# ---------------------------
...
def extrude_surface_z_solid(tri_faces, depth):
    ...
    for ia, ib, ic in tris_idx:
        ...
        out.append((back_verts[ic], back_verts[ib], back_verts[ia]))  # back flipped
...
# ---------------------------
# Anchor ribs (FAST booleans to avoid GMP spam)
# ---------------------------
...
def apply_boolean_union(target_obj, cutters):
    ...
    for cutter in cutters:
        ...
        mod.operation = 'UNION'
        mod.solver = 'FAST'  # avoid EXACT/GMP warnings
        mod.object = cutter
        try:
...
def export_stl_selected(filepath, *, global_scale: float = 1000.0):
    # why: STL consumers assume mm; we model in meters.
...
def main():
    ...
    # Export scaling: default mm; override with params.stlUnits or params.stlScale
